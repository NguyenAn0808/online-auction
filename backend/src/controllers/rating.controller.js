...
class RatingController {
  ...
  static async addRating(req, res) {
    try {
      const { product_id, reviewer_id, target_user_id, score, comment } =
        req.body;
  ...
  static async getRatingsGivenByUser(req, res) {
    try {
      const { user_id } = req.params;
      if (!user_id)
        return res
          .status(400)
          .json({ success: false, message: "Missing user_id" });
      const result = await RatingService.getRatingsGivenByUser(user_id);
      return res.status(200).json({ success: true, data: result });
    } catch (error) {
      return res.status(500).json({ success: false, message: error.message });
    }
